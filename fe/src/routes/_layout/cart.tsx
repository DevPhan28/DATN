...
function Cart() {
  ...
  const incrementQuantity = (index: number) => {
    ...
    if (product) {
      increaseQuantity.mutate({ userId: userId || '', productId: product.productId, variantId: product.variantId });
    }
  };
  ...
  const decrementQuantity = (index: number) => {
    ...
    if (product && newQuantity > 0) {
      decreaseQuantity.mutate({ userId: userId || '', productId: product.productId, variantId: product.variantId });
    }
  };
        ...
        <div className="w-full flex-none md:w-[70%]">
          ...
          <div className="border-t p-4">
            <div className="flex justify-around">
              <input type="text" placeholder="discount code" className="rounded-md border p-2 focus:outline-none" />
              <button className="rounded-md bg-gray-300 px-4 py-2 text-black hover:bg-blue-600">apply code</button>
              <button className="rounded-md bg-gray-300 px-4 py-2 hover:bg-gray-400">Update Cart</button>
            </div>
          </div>
        </div>

        <div className="w-full rounded-md border p-10 md:w-[30%] lg:max-w-lg">
          <h2 className="mb-4 whitespace-nowrap text-xl font-bold uppercase">Cart Totals</h2>
          <div className="mb-2 flex items-center justify-between">
            <span className="font-medium">Subtotal:</span>
            <span className="font-bold">${totalSelectedPrice}</span>
          </div>
          <button onClick={handleCheckout} className="w-full rounded-md bg-blue-400 px-6 py-3 text-white hover:bg-blue-600">Proceed to Checkout</button>
        </div>
      </div>
    </div>
  );
